...
std::string trim_copy_ascii(std::string text)
{
...
bool parse_grbl_properties_snapshot(std::string const &text, GrblFirmwareSnapshot &snapshot, Glib::ustring &error)
{
    ...
    while (std::getline(input, line)) {
        ...
        auto trimmed = trim_copy_ascii(line);
        ...
        auto const key = trim_copy_ascii(trimmed.substr(0, eq));
        auto const value = trim_copy_ascii(trimmed.substr(eq + 1));
        ...
        try {
            if (code == 3) {
                size_t idx = 0;
                snapshot.direction_mask = std::stoi(value, &idx);
                if (idx == value.size()) {
                    snapshot.has_direction_mask = true;
                }
            } else if (code == 130) {
                size_t idx = 0;
                snapshot.x_travel_mm = std::stod(value, &idx);
                if (idx == value.size()) {
                    snapshot.has_x_travel = true;
                }
            } else if (code == 131) {
                size_t idx = 0;
                snapshot.y_travel_mm = std::stod(value, &idx);
                if (idx == value.size()) {
...
GrblControlPanel::GrblControlPanel()
    ...
    , _chk_hatch_inset(_("排线前先内缩轮廓"))
    , _chk_hatch_angle_increment(_("每个轮廓递增排线角度"))
...
void GrblControlPanel::update_mapping_control_sensitivity(bool const allow_interaction)
{
    ...
    auto const state = make_grbl_panel_mapping_sensitivity(
        ...
        _chk_contour_to_hatch.get_active(), _chk_hatch_inset.get_active(), _chk_hatch_angle_increment.get_active(),
    ...
    _chk_hatch_inset.set_sensitive(state.hatch_inset_enable);
    _chk_hatch_angle_increment.set_sensitive(state.hatch_angle_increment_enable);
    ...
    _hatch_inset_spin.set_sensitive(state.hatch_inset);
    _hatch_angle_increment_spin.set_sensitive(state.hatch_angle_increment);
...
void GrblControlPanel::apply_mapping_preferences_to_ui(GrblPanelMappingPrefs const &values)
{
    ...
    _chk_hatch_inset.set_active(values.hatch_inset_enable);
    _chk_hatch_angle_increment.set_active(values.hatch_angle_increment_enable);
    ...
    _hatch_inset_spin.set_value(values.hatch_inset);
...
GrblPanelMappingPrefs GrblControlPanel::read_mapping_preferences_from_ui() const
{
    ...
    values.hatch_inset_enable = _chk_hatch_inset.get_active();
    values.hatch_angle_increment_enable = _chk_hatch_angle_increment.get_active();
    ...
    values.hatch_inset = _hatch_inset_spin.get_value();
...
void GrblControlPanel::build_ui()
{
    ...
    _chk_sparse_sampling.set_tooltip_text(_("对规则排线或密集线场做快速抽稀：按笔画顺序每隔 N 条保留 1 条，减少发黑和绘制时间。它会主动少画一部分线，不是纯路径优化。"));
    ...
    _chk_lead_in.set_tooltip_text(_("参考 GRBL-Plotter 的 lead-in：对开放路径在起笔前延伸一小段，把落笔过渡尽量放到图形外。它会改动开放路径的首端几何，不是纯路径排序。"));
    _chk_lead_out.set_tooltip_text(_("参考 GRBL-Plotter 的 lead-out：对开放路径在收笔后延伸一小段，把抬笔过渡尽量放到图形外。它会改动开放路径的末端几何，不是纯路径排序。"));
    _lead_in_dist_spin.set_digits(2);
    _lead_in_dist_spin.set_range(0.0, 1000.0);
    _lead_in_dist_spin.set_increments(0.05, 0.5);
    _lead_in_dist_spin.set_tooltip_text(_("开放路径起笔前延伸的距离。单位 mm；闭合轮廓不会应用。"));
    _lead_out_dist_spin.set_digits(2);
    ...
    _chk_hatch_inset.set_tooltip_text(_("参考 GRBL-Plotter 的 inset：先把闭合轮廓向内缩一点，再生成排线，减少排线贴边或越界到边缘的情况。"));
    _chk_hatch_angle_increment.set_tooltip_text(_("每处理完一个闭合轮廓，就把下一块排线的角度再偏移一次。适合相邻区域做错角填充。"));
    ...
    _hatch_inset_spin.set_digits(2);
    _hatch_inset_spin.set_range(0.0, 100.0);
    _hatch_inset_spin.set_increments(0.05, 0.5);
    _hatch_inset_spin.set_tooltip_text(_("内缩距离。单位 mm；如果内缩后轮廓退化或无效，会自动回退到原轮廓。"));
    _hatch_angle_increment_spin.set_digits(1);
    ...
    _near_connect_dist_spin.set_tooltip_text(_("当相邻两段笔画的间距不大于这个值时，直接不断笔连过去。单位 mm；这段桥接线会真正出现在最终 G-code 中。建议先从 0.1–0.5 mm 小范围试起。"));
    ...
    _sparse_keep_every_spin.set_tooltip_text(_("抽稀步长。1 表示不过滤，2 表示隔 1 条留 1 条，3 表示每 3 条保留 1 条；保留下来的笔画会真正变少。"));
    ...
    job_tuning_grid->attach(_chk_lead_in, 0, 1, 1, 1);
    auto *lbl_lead_in = Gtk::make_managed<Gtk::Label>(_("起笔距离(mm)"), Gtk::Align::START);
    job_tuning_grid->attach(*lbl_lead_in, 1, 1, 1, 1);
    job_tuning_grid->attach(_lead_in_dist_spin, 2, 1, 1, 1);
    job_tuning_grid->attach(_chk_lead_out, 0, 2, 1, 1);
    ...
    job_tuning_grid->attach(_chk_hatch_inset, 0, 5, 1, 1);
    auto *lbl_hatch_inset = Gtk::make_managed<Gtk::Label>(_("内缩距离(mm)"), Gtk::Align::START);
    job_tuning_grid->attach(*lbl_hatch_inset, 1, 5, 1, 1);
    job_tuning_grid->attach(_hatch_inset_spin, 2, 5, 1, 1);
    job_tuning_grid->attach(_chk_hatch_angle_increment, 0, 6, 1, 1);
    auto *lbl_hatch_angle_increment = Gtk::make_managed<Gtk::Label>(_("递增角度(°)"), Gtk::Align::START);
    job_tuning_grid->attach(*lbl_hatch_angle_increment, 1, 6, 1, 1);
    job_tuning_grid->attach(_hatch_angle_increment_spin, 2, 6, 1, 1);
    job_tuning_grid->attach(_chk_sparse_sampling, 0, 7, 1, 1);
    auto *lbl_sparse_sampling = Gtk::make_managed<Gtk::Label>(_("每隔 N 条留 1 条"), Gtk::Align::START);
    job_tuning_grid->attach(*lbl_sparse_sampling, 1, 7, 1, 1);
    job_tuning_grid->attach(_sparse_keep_every_spin, 2, 7, 1, 1);
    auto *lbl_tool_change_mode = Gtk::make_managed<Gtk::Label>(_("换笔模式"), Gtk::Align::START);
    job_tuning_grid->attach(*lbl_tool_change_mode, 0, 8, 1, 1);
    job_tuning_grid->attach(_tool_change_mode_combo, 1, 8, 2, 1);
    job_tuning_grid->attach(_chk_manual_pen_change_to_home, 0, 9, 2, 1);
    job_tuning_grid->attach(_chk_manual_pen_change_prompt, 0, 10, 2, 1);
    job_tuning_grid->attach(_chk_tool_change_point, 0, 11, 1, 1);
    ...
    job_tuning_grid->attach(*tool_change_xy_box, 1, 11, 2, 1);
